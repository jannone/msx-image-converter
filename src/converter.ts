...
import { Image, LabColor, RGBColor } from './common'
...
export const convert = (inputImage: Image, outputImage: Image, palette: RGBColor[]) => {
  ...
  const labPalette = palette.map(c => RGBToLab(c))
  ...
  while (y < inputImage.height) {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const pixel = RGBToLab(getPixel(inputImage, x + i, y))
      octet.push(pixel)
    }
    ...
    let bestcolor1: number = 0
    let bestcolor2: number = 0
    const octetfinal = [0, 0, 0, 0, 0, 0, 0, 0]
    for (let c1 = 1; c1 < 16; c1++) {
      for (let c2 = c1; c2 < 16; c2++) {
        ...
        const tone = [
          ...
          RGBToLab(mixedColor(palette[c1], palette[c2])),
