...
export const contrast = (inputImage: Image, outputImage: Image, amount: number) => {
  if (amount < -255 || amount > 255) {
    throw new Error('Contrast amount must be in the interval of [-255, 255]')
  }
  const factor = (259 * (amount + 255)) / (255 * (259 - amount))
  const apply = (value: number, scale: number) => clamp(scale * (value - 128) + 128)
...
export const kernelBlur = (value: number): ConvolutionKernel => {
  ...
  return [offset, offset, offset, offset, offset, offset, offset, offset, offset]
}
